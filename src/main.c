...
/*   Updated: 2019/02/12 18:37:27 by arudyi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_change_player_dest(t_elem *s_pixel, int key)
{
	...
	if (key == 2 && ft_is_wall(s_pixel, s_pixel->player->x_camera + 15, s_pixel->player->y_camera))
	{
		s_pixel->player->x_camera += 10;
	}
	else if (key == 0 && ft_is_wall(s_pixel, s_pixel->player->x_camera - 15, s_pixel->player->y_camera))
	{
		s_pixel->player->x_camera -= 10;
	}
	else if (key == 1 && ft_is_wall(s_pixel, s_pixel->player->x_camera, s_pixel->player->y_camera + 15))
	{
		s_pixel->player->y_camera += 10;
	}
	else if (key == 13 && ft_is_wall(s_pixel, s_pixel->player->x_camera, s_pixel->player->y_camera - 15))
	{
		s_pixel->player->y_camera -= 10;
		//s_pixel->player->x_camera *= sin(s_pixel->player->pov / 180 * 3.14);
	}
}
...
int ft_is_wall(t_elem *s_pixel, int x, int y)
{
	...
	index = s_pixel->size_line * y + x * 4;
	if (*(unsigned *)(s_pixel->begin_str1 + index) == 0x0000FF)
		return (0);
	return (1);
}

int ft_check_gor(t_elem *s_pixel, int dir)
{
	short int	is_inside;
	double	ax;
	double	ay;
...
int ft_check_ver(t_elem *s_pixel, int dir)
{
	int	is_inside;
	double	bx;
	double	by;
	...
	if (dir == 270 || dir == 90)
	{
		s_pixel->walls->dot_wall_x = -404;
		s_pixel->walls->dot_wall_y = -404;
		return (0);
	}
	px = (double)s_pixel->player->x_camera;
	py = (double)s_pixel->player->y_camera;
	is_inside = 1;
	bx = ((0 <= dir && dir <= 90) || (270 < dir && dir <= 360)) ? (int)px / 64 * 64 + 64 : (int)px / 64 * 64 - 1;
	by = (dir == 90 || dir == 270) ? py : py + (px - bx) * tan((double)dir / 180 * 3.14);
	xa = ((0 <= dir && dir <= 90) || (270 <= dir && dir <= 360)) ? 64 : -64;
...
void ft_find_wall(t_elem *s_pixel, int dir)
{
	...
	//s_pixel->walls->len_vec *= cos((double)dir / 180 * 3.14);
	//s_pixel->walls->len_vec *= (s_pixel->walls->len_vec < 0) ? -1 : 1;
...
void ft_draw_walls(t_elem *s_pixel, int x)
{
	...
	k = 32;
	len = 64 / s_pixel->walls->len_vec * 1662;
	...
	while (k)
	{
		...
		while (i <= len)
		{
			if (x > 1920 || y > 960)
				...
				ft_pixel_to_image(s_pixel, x , y, 0x0000FF);
			y++;
			i++;
		}
		x++;
